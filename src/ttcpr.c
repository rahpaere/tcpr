...
int tcprfd;			/* fd of TCPR control socket */
struct tcpr_ip4 tcprstate;
...
int usetcpr = 0;		/* protect connection with TCPR */
int sendcheckpoints = 1;	/* send TCPR checkpoints */
...
Common options:\n\
	...
	-P	protect connection with TCPR\n\
	-C	do not send TCPR checkpoints (with -P)\n\
...
char **argv;
{
	...
	while ((c = getopt(argc, argv, "drstuvBDTb:f:PCl:n:p:A:O:R")) != -1) {
		switch (c) {
...
"ttcp: -b option ignored: SO_SNDBUF/SO_RCVBUF socket options not supported\n");
	...
	if (usetcpr) {
		int len = sizeof(sinme);
		getsockname(fd, (struct sockaddr *)&sinme, &len);
		getpeername(fd, (struct sockaddr *)&sinhim, &len);
		if ((tcprfd = socket(AF_INET, SOCK_DGRAM, 0)) < 0)
			err("socket");
		if (connect(tcprfd, (struct sockaddr *)&sinhim, sizeof(sinhim)) < 0)
			err("connect");
		tcprstate.address = sinme.sin_addr.s_addr;
		tcprstate.peer_address = sinhim.sin_addr.s_addr;
		tcprstate.tcpr.hard.port = sinme.sin_port;
		tcprstate.tcpr.hard.peer.port = sinhim.sin_port;
		if (send(tcprfd, &tcprstate, sizeof(tcprstate), 0) < 0)
			err("send");
		if (recv(tcprfd, &tcprstate, sizeof(tcprstate), 0) < 0)
			err("recv");
		if (!sendcheckpoints) {
			tcprstate.tcpr.hard.done_reading = 1;
			if (send(tcprfd, &tcprstate, sizeof(tcprstate), 0) < 0)
				err("send");
		}
	}
	...
	if (usetcpr) {
		tcprstate.tcpr.hard.done_reading = 1;
		tcprstate.tcpr.hard.done_writing = 1;
		if (send(tcprfd, &tcprstate, sizeof(tcprstate), 0) < 0)
			err("send");
	}
	exit(0);
...
int count;
{
	...
	if( udp )  {
		...
		if (sendcheckpoints) {
			tcprstate.tcpr.hard.ack = htonl(ntohl(tcprstate.tcpr.hard.ack) + cnt);
			if (send(tcprfd, &tcprstate, sizeof(tcprstate), 0) < 0)
				err("send");
		}
	}
